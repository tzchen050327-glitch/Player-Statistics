(() => {
  ...
  function defenseMap(detail, side) {
    ...
    // Re-apply official CPBL defensive changes at render time. The upstream
    // fielders array can reflect only part of a multi-player switch (for example
    // moving the starting 2B to SS while omitting the pinch runner who then takes
    // over 2B). Replaying the change log makes the landscape defense authoritative.
    if (String(detail?.league || '').toUpperCase() === 'CPBL') {
      const roleAndName = value => {
        ...
        let match = text.match(new RegExp(`^${role}[-：:]?(.*)(() => {
  const UI_VERSION = document.querySelector('meta[name="app-version"]')?.getAttribute('content') || 'v2.78';
  const DETAIL_URL_RE = /\/(?:league-game-detail|cpbl-game-detail|cpbl-postseason-detail|npb-game-detail)(?:\?|$)/i;
  const CPBL_PREGAME_LINEUP_URL = 'https://kjndnsztbcpmkhictjkr.supabase.co/functions/v1/cpbl-pregame-lineup';
  const CPBL_APP_KEY = 'TyPAf0puXo-lBcrIf4Ky1wQryHaG2f4j';
...
, 'i'));
        ...
        match = text.match(new RegExp(`^(.*?)[-：:]?${role}(() => {
  const UI_VERSION = document.querySelector('meta[name="app-version"]')?.getAttribute('content') || 'v2.78';
  const DETAIL_URL_RE = /\/(?:league-game-detail|cpbl-game-detail|cpbl-postseason-detail|npb-game-detail)(?:\?|$)/i;
  const CPBL_PREGAME_LINEUP_URL = 'https://kjndnsztbcpmkhictjkr.supabase.co/functions/v1/cpbl-pregame-lineup';
  const CPBL_APP_KEY = 'TyPAf0puXo-lBcrIf4Ky1wQryHaG2f4j';
